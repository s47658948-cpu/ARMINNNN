...
export async function handler(event) {
  ...
  try {
    ...
    if (event.httpMethod === "POST" && action === "ticket-reply") {
      ...
      const now=Date.now();
      if(username){
        if(normalizeUsername(ticket.username)!==username) return reply(403,{ok:false,error:"شما فقط می‌توانید به تیکت خودتان پیام دهید."});
        await db("ticket_messages",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({id:crypto.randomUUID(),ticket_id:id,sender:"user",sender_name:ticket.name||username,body:text,created_at:now})});
        await db(`tickets?id=eq.${encodeURIComponent(id)}`,{method:"PATCH",body:JSON.stringify({status:"open",updated_at:now})});
